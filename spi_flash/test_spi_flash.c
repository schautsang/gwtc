...
void __end_of_tx(void *arg)
{
    ...
    pi_flash_read_async(&flash, spi_buff, rcv_buff, (uint32_t) BUFFER_SIZE, cb);
}
...
void __end_of_erase(void *arg)
{
    ...
    pi_flash_program_async(&flash, spi_buff, buff, (uint32_t) BUFFER_SIZE, cb);
}
#endif  /* ASYNC */
...
void test_spi_flash(void)
{
    ...
    //spi_buff = ((flash_info.flash_start + flash_info.sector_size - 1) &
    ...
    pi_flash_erase_async(&flash, spi_buff, (uint32_t) BUFFER_SIZE, &cb_erase);
    while (!done)
    {
        pi_yield();
    }
    #else
    pi_flash_erase(&flash, spi_buff, (uint32_t) BUFFER_SIZE);
    ...
    printf("\nspiflash transfer done with %ld error(s) !\n", errors);
