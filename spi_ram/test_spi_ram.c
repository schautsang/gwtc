...
void __end_of_tx(void *arg)
{
    ...
    pi_ram_read_async(&ram, spi_buff, rcv_buff, (uint32_t) BUFFER_SIZE, cb);
}
#endif  /* ASYNC */
...
void test_spi_ram(void)
{
    ...
    #else
    pi_ram_write(&ram, spi_buff, buff, (uint32_t) BUFFER_SIZE);
    printf("Write sync done.\n");
    pi_ram_read(&ram, spi_buff, rcv_buff, (uint32_t) BUFFER_SIZE);
